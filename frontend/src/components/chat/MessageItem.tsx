...
const MessageItem = ({ message, index, messages, selectedConvo, lastMessageStatus }: MessageItemProps) => {
    ...
    const isShowTime = index === 0 ||
        new Date(message.createdAt).getTime() - new Date(prev?.createdAt || 0).getTime() > 300000; // tách group tin nhắn
    
    const isGroupBreak = isShowTime ||
        message.senderId !== prev?.senderId;
    // tin nhắn đầu tin luôn tách nhóm
    // tin này với tin trước đó khác người gửi thì tách nhóm
    // nếu tin nhắn cách nhau trên 5p, thì tách nhóm và hiện lại avatar
